...
void GamePlayScene::KeyUpdate(float fTimeElapsed) {
	...
	if (m_keys.space == true && m_gamestatus[m_pGameClient->m_ClientNum].flag.space_flag == 0) {
		hard_drop_block();
		score += m_gamestatus[m_pGameClient->m_ClientNum].level; // hard drop 보너스
...
void GamePlayScene::drop_block(float fTimeElapsed) {
	...
	if (m_gamestatus[m_pGameClient->m_ClientNum].fDropBlockTime >= m_gamestatus[m_pGameClient->m_ClientNum].speed) {
		if (check_crush(m_gamestatus[m_pGameClient->m_ClientNum].bx, m_gamestatus[m_pGameClient->m_ClientNum].by + 1, m_gamestatus[m_pGameClient->m_ClientNum].b_rotation) == false) { //밑이 비어있지않고 crush m_gamestatus[m_pGameClient->m_ClientNum].flag가 켜저있으면
			...
			m_gamestatus[m_pGameClient->m_ClientNum].flag.crush_on = false; //m_gamestatus[m_pGameClient->m_ClientNum].flag를 끔
			check_line(); //라인체크를 함
			m_gamestatus[m_pGameClient->m_ClientNum].flag.new_block_on = true; //새로운 블럭생성 m_gamestatus[m_pGameClient->m_ClientNum].flag를 켬
			return;
		}
		if (check_crush(m_gamestatus[m_pGameClient->m_ClientNum].bx, m_gamestatus[m_pGameClient->m_ClientNum].by + 1, m_gamestatus[m_pGameClient->m_ClientNum].b_rotation) == true)
		...
		if (check_crush(m_gamestatus[m_pGameClient->m_ClientNum].bx, m_gamestatus[m_pGameClient->m_ClientNum].by + 1, m_gamestatus[m_pGameClient->m_ClientNum].b_rotation) == false)
			m_gamestatus[m_pGameClient->m_ClientNum].flag.crush_on = true; //밑으로 이동이 안되면  crush m_gamestatus[m_pGameClient->m_ClientNum].flag를 켬
...
void GamePlayScene::move_block(int dir) { //블록을 이동시킴 
	...
	case LEFT: //왼쪽방향 
		...
		m_gamestatus[m_pGameClient->m_ClientNum].bx--; //좌표값 이동 
		break;
	...
	case UP: //키보드 위쪽 눌렀을때 회전시킴. 
		...
		m_gamestatus[m_pGameClient->m_ClientNum].b_rotation = (m_gamestatus[m_pGameClient->m_ClientNum].b_rotation + 1) % 4; //회전값을 1증가시킴(3에서 4가 되는 경우는 0으로 되돌림) 
...
void GamePlayScene::check_line(void) {
	...
	for (i = BOARD_Y - 2; i > 3;) { //i=MAIN_Y-2 : 밑쪽벽의 윗칸부터,  i>3 : 천장(3)아래까지 검사 
		...
		if (block_amount == BOARD_X - 2) { //블록이 가득 찬 경우 
			if (m_gamestatus[m_pGameClient->m_ClientNum].flag.level_up_on == 0) { //레벨업상태가 아닌 경우에(레벨업이 되면 자동 줄삭제가 있음) 
				score += 100 * m_gamestatus[m_pGameClient->m_ClientNum].level; //점수추가 
				// cnt++; //지운 줄 갯수 카운트 증가 
				combo++; //콤보수 증가  
			}
	...
	if (combo) { //줄 삭제가 있는 경우 점수와 레벨 목표를 새로 표시함  
		if (combo > 1) { //2콤보이상인 경우 경우 보너스및 메세지를 게임판에 띄웠다가 지움 
			...
			reset_main_cpy(); //텍스트를 지우기 위해 m_gamestatus[m_pGameClient->m_ClientNum].board_cpy을 초기화.
		//(m_gamestatus[m_pGameClient->m_ClientNum].board_cpy와 m_gamestatus[m_pGameClient->m_ClientNum].board_org가 전부 다르므로 다음번 draw()호출시 게임판 전체를 새로 그리게 됨) 
		}
...
void GamePlayScene::check_level_up(void) {
	...
	if (cnt >= 10) { //레벨별로 10줄씩 없애야함. 10줄이상 없앤 경우 
		draw_main();
		m_gamestatus[m_pGameClient->m_ClientNum].flag.level_up_on = 1; //레벨업 m_gamestatus[m_pGameClient->m_ClientNum].flag를 띄움
		m_gamestatus[m_pGameClient->m_ClientNum].level += 1; //레벨을 1 올림
		cnt = 0; //지운 줄수 초기화  

		reset_main_cpy(); //텍스트를 지우기 위해 m_gamestatus[m_pGameClient->m_ClientNum].board_cpy을 초기화.
		//(m_gamestatus[m_pGameClient->m_ClientNum].board_cpy와 m_gamestatus[m_pGameClient->m_ClientNum].board_org가 전부 다르므로 다음번 draw()호출시 게임판 전체를 새로 그리게 됨) 

		//.check_line()함수 내부에서 m_gamestatus[m_pGameClient->m_ClientNum].level up m_gamestatus[m_pGameClient->m_ClientNum].flag가 켜져있는 경우 점수는 없음.         
		switch (m_gamestatus[m_pGameClient->m_ClientNum].level) { //레벨별로 속도를 조절해줌. 
		case 2:
		...
		m_gamestatus[m_pGameClient->m_ClientNum].flag.level_up_on = 0; //레벨업 m_gamestatus[m_pGameClient->m_ClientNum].flag꺼줌

		gotoxy(STATUS_X_ADJ, STATUS_Y_LEVEL); printf(" LEVEL : %5d", m_gamestatus[m_pGameClient->m_ClientNum].level); //레벨표시 
